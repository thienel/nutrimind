...
import { TrendingDown, Utensils, Droplets, Scale, ChevronRight } from "lucide-react-native";
...
export default function ProgressScreen() {
  ...
  const maxCal = Math.max(...calorieHistory.map((d) => d.calories), CALORIE_GOAL);
      ...
      >
        ...
        {weightChange != null && (
          <View style={styles.weightChangeCard}>
            <Scale size={18} color={weightChange <= 0 ? "#10B981" : "#EF4444"} />
          ...
          {calorieHistory.length > 0 ? (
            <View style={styles.barChart}>
              {calorieHistory.map((d, i) => {
                ...
                const dayLabel = new Date(d.date).toLocaleDateString("vi-VN", {
                  weekday: "short",
                });
                return (
        ...
        <View style={styles.linksCard}>
          ...
          <QuickLink
            ...
            sub={latestWeight != null ? `${latestWeight.toFixed(1)} kg hiện tại` : "Chưa có dữ liệu"}
...
function SummaryCard({
  icon, bg, label, value, unit, valueColor,
}: {
...
function QuickLink({
  icon, bg, label, sub, onPress, isLast,
}: {
  icon: React.ReactNode;
  bg: string;
  label: string;
...
const styles = StyleSheet.create({
  ...
  goalHint: { fontSize: 11, color: "#94A3B8", marginTop: 8, textAlign: "center" },
